...
if sampling != -1:
    savename = 'plot/' + name + '_sampling_' + str(sampling) + '_opt_angle.pdf'
...
opt_freq_time, opt_freq, opt_angle = lib_smb.get_opt_freq(num_slots, time, enco)
opt_freq_rel_time = lib_smb.get_rel_time(opt_freq_time)

# get driver frequency
driver_freq_time, driver_freq, driver_angle = lib_smb.get_driver_freq(time, driver)
...
# plt.plot(driver_freq_rel_time, driver_angle, 'o', linewidth = 2.0, label='driver', markerfacecolor = 'plum')
# plt.plot(opt_freq_rel_time, opt_angle, 'or', linewidth = 2.0, label='opt. enc.')

plt.plot(driver_freq_rel_time, driver_angle, 'o', linewidth = 2.0, label='driver', markerfacecolor = 'lightgreen')
plt.plot(opt_freq_rel_time, opt_angle, 'og', linewidth = 2.0, label='opt. enc.')
